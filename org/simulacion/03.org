#+title:Simulación de eventos aleatorios
* Probabilidades y variables aleatorias
** Conceptos básicos de Teoría de Probabilidad

   Experimento aleatorio: al repetirlo bajo las mismas condiciones, el resultado
   (mediciones) varía de manera impredecible.

   - Ej. 1: Tirar una moneda 50 veces y anotar el número de “caras” obtenidos.
   - Ej. 2: Contar el número de vehículos en un estacionamiento los Lunes a las 10AM.
   - Ej. 3: Medir el tiempo entre llamadas telefónicas.
   - Ej. 4: Medir el tiempo entre activaciones (spikes) de una neurona.

   Espacio muestral (S): posibles resultados del experimento aleatorio.
   - Ej. 1: $S = \{0,1,2,\dots,50\}$
   - Ej. 2: $S = \{0,1,2,\dots, Max\}$
   - Ej. 3: $S = 0 < t < T_{max}$
   - Ej. 4: $S = 0 < t < T_{max}$

   Definición axiomática de probabilidad

   $A, B \subset S$

   1. $0 \leq P(A)$
   2. $P(S) = 1$
   3. if $A \cap B = \emptyset$, then $P(A \cup B) = P(A) + P(B)$

** Variables Aleatorias

   | Discretas                            | Continuas                             |
   | Espacio muestral contable            | Espacio muestral no-contable          |
   | S = {x1, x2, \dots}                  |                                       |
   | Distribución de probabilidades p(xi) | Función densidad (pdf) p(x)           |
   | x1 x2 x3 x4                          |                                       |
   | Cálculo de probabilidades: sumando   | Cálculo de probabilidades: Integrando |
   | $P(A) = \sum_{x_i \in A} p(x_i)$     | $P(A) = \int_{A}p(x)dx$               |

   | Bernoulli Random Variable               | Uniform Random Variable                  |
   | S_{X} = \{0,1\}                         | S_{X} = [a,b]                            |
   | p_0 = q = 1-p, p_1 = p, 0 \leq p \leq 1 | f_{X}(x)= \frac{1}{b-a}, a \leq x \leq b |
   |                                         |                                          |

* Generación de números aleatorios

** Generación de números aleatorios

*** Números realmente aleatorios:
    Generados a partir de fuentes físicas:
    - Tirando dados / Utilizando urnas
    - Midiendo ruido en semiconductores

    Datos históricos:
    - Antes de las computadoras, se publicaban libros con secuencias de dígitos
      aleatorios.

    Por ej. 40,000 números en 1927 (Tippett) y 100,000 números en 1939 (KendalBabington Smith).
    - La computadora Ferranti Mark I (1951) generaba una cadena de 20 bits aleatoria
      usando el ruido de una resistencia eléctrica.

*** Números pseudo-aleatorios:
    No es práctico utilizar mecanismos físicos para generar números por lo que se
    han desarrollado algoritmos determinísticos para generar secuencias de números
    que parezcan aleatorios.

    John Von Neuman algorithm (1946)

    Dada una semilla de N dígitos, elevarlo al cuadrado y extraer los N dígitos
    centrales. Repetir el proceso. Por ejemplo, con N=10:

    5772156649 -> "33317 *792380594* 909201" [label="^2"]
    "33317 *792380594* 909201" -> 7923805949 [label="^2"]

    Problema: Degeneración. Si se genera una secuencia de 0s o se repite un número.

    Metropolis obtuvo una secuencia de 750,000 números (período) usando N=38 (1956)


** Propiedades de generadores “buenos”
   - Los números deben tener una distribución correcta, por ejemplo uniforme, y no
     deben estar correlacionados.
   - La secuencia debe tener un período suficientemente largo.
   - La secuencia se tiene que poder reproducir (replicar experimentos).

*** Generación de números aleatorios uniformes [0,1]

    Generamos números enteros Xn en [0,m] y calculamos: Un = Xn/m

** Método Lineal Congruente

   Es de la forma $X_{n+1} = f(X_n)$

   Método Lineal Congruente [Lehmer, 1949]
   $X_{n+1} = \left(aX_n + c\right) mod m, n \geq 0$
   - $0 < m$, the modulus
   - $0 \leq a < m$, the multiplier
   - $0 \leq c < m$, the increment
   - $0 \leq X_0 < m$, the starting value

   Por ejemplo:
   $m=10,X_0=a=c=7$
   $7, 6, 9, 0, 7, 6, 9, 0, \dots$

   Período = 4

   Ejercicio: Demostrar que el caso a=0 no es admisible

   Cómo asegurar que el período sea el máximo posible m?

   Teorema A [Grinberger, 1961; Hull&Dobel, 1962]:

   El algoritmo Lineal Congruente tiene período m si y sólo sí:
   1) c es relativo primo con m (no existe número k ≠ 1 que divida a ambos)
   2) b = a - 1 es un múltiplo de p, para cada primo p dividiendo a m.
   3) b es un múltiplo de 4, si m es un múltiplo de 4

   Caso particular que cumple el teorema:
   a=2^k+1
   m=2^e
   c=1
   2 \leq k < e

   Ejercicio: Demostrar que se cumplen las condiciones del Teorema A para este
   caso

** Ejemplos (buenos y malos)

   IBM’s RANDU (1960’):

   X_{n+1} = (65539 X_n ) mod 231

   MALO: No pasa tests simples de aleatoriedad (ya lo veremos)

   GGL (Lewis-Goodman-Miller, 1969):

   X_{n+1} = (16087X_n )  mod (2^{31}- 1)

   Nota: Remplazó a RANDU en IBM 360. Usado originalmente en Matlab

   VAX’s RAND (1977):

   X_{n+1} = (69069 X_n + 1)   mod (2^{32})

   Ejercicio: Demostrar si se cumplen las condiciones del Teorema A para estos casos

** Otros Métodos

   Cuadrático:
   X_{n+1} = \left(dX^2_n + aX_n +c \right) mod m

   Caso particular (Coveyou):

   X_{n+1} = X_{n}(X_{n} + 1) mod 2^{e} con X_0 mod 4 = 2

   Considerando varios valores del “pasado”

   Secuencia de Fibonacci:
   X_{n+1} = (X_{n} + X_{n-1})  mod m

   [Green-Smith-Klem, 1959]
   X_{n+1} = (X_{n} + X_{n-k})  mod m


   [Mitchell-Moore, 1958]
   X_{n+1} = (X_{n-24} + X_{n-55})  mod m


   Nota: Considerar varios valores del “pasado” permite obtener secuencias con períodos
   mucho mas largos que m

**   Métodos Avanzados

   Mersenne Twister [Matsumoto&Nishimura, 1998]:

   Método por default en Matlab

   Números primos de Mersenne:

   2^{p}-1 , con p primo

   En este método p = 19937 (período igual a (2^{19937}- 1)!!! )

   2^{10}- 1 = 1, 023

   2^{100}-   1 = 1.2677e + 30

   2^{1000} -  1 = 1.0715e + 301

   2^{19937}-1 \approx  1.0e + 6020

   Nota: El número de átomos en el Universo se estima en un valor entre 1078 y 1082!!!
   Algoritmo: Involucra solo operaciones sobre registros binarios de tipo SHIFT, AND, OR y XOR.

* Tests de Aleatoriedad
   - Tests teóricos (de hipótesis)
   - Tests empíricos
** Test de hipótesis (significancia)

   Problema: Supongamos que queremos saber si un conjunto de N números
   X = {x1, x2, \dots xN} responde a una determinada distribución de probabilidades pX(x).

   Ejemplo: Se tira un dado 100 veces y se anotan las ocurrencias de cada
   número. Pregunta: Está cargado el dado?

   Hipótesis nula H0: pi = 1/6, i=1,2,\dots,6.
   Queremos un criterio estadístico para aceptar o rechazar la hipótesis.
   Hay un costo en rechazar H0 si fuera verdadera y lo podemos cuantificar
   calculando la probabilidad asociada a este error:
   E: espacio muestral
   (todos los posibles X)
   Ac

   A
   <latexit sha1_base64="weGK+LMZ9g1IFC1dRNF61/5Mw1c="></latexit>

   En nuestro ejemplo pX(X|H0)
   sería la distribución multinomial.

   A: casos para los cuales aceptamos H0
   A^{c}: casos para los cuales rechazamos H0

   Nivel de significancia : representa nuestra tolerancia para aceptar errores. Diseñamos la partición
   del espacio muestral de manera que la probabilidad del error se encuentre por debajo del nivel de
   significancia.
   Generalmente se usan valores
   o
   .
   <latexit sha1_base64="JtDqaCSYHdUsArJlViGZOYtHm8o="></latexit>

   <latexit sha1_base64="u2s30bk8UnapK+1NXxs0IphSIY4="></latexit>

   <latexit sha1_base64="Il2UcSmSsjsl3ZU7D+GoaIYbs9Q="></latexit>





** Caso discreta: Test Estadístico Chi2

   15

   Cuando se aplica?
   Cuando tenemos una muestra de una variable
   aleatoria discreta finita y queremos evaluar si
   responde a una determinada distribución de
   probabilidad teórica.

   <latexit sha1_base64="bkCP3I9YJWa0WIq5ymXb46EMsA8="></latexit>

   <latexit sha1_base64="ITy9EE2YiceosOIWTBDZM5Bxwh4="></latexit>

   Ejemplo (libro Leon-Garcia): se toman n=114
   números al azar de la guía telefónica y se anotan
   los últimos dígitos. Queremos saber si la
   distribución es uniforme.

   Método: medimos la dispersión de las ocurrencias observadas (
   esperadas (
   ):
   <latexit sha1_base64="bkCP3I9YJWa0WIq5ymXb46EMsA8="></latexit>

   ) respecto de las

   <latexit sha1_base64="ITy9EE2YiceosOIWTBDZM5Bxwh4="></latexit>

   Pearson (1900) demostró que D2 responde a una distribución
   de tipo Chi2, por lo tanto, encontramos tal que
   y aceptamos la hipótesis siempre que
   <latexit sha1_base64="fInOqGTCCrWkRGJFOZWK1l6FLBY="></latexit>

   <latexit sha1_base64="lccRgaShPhZ69tPojm9FdfGGMi8="></latexit>

   <latexit sha1_base64="8pSi+1AgvpeGonMtEfe7uG3/L7M="></latexit>

   <latexit sha1_base64="0Q5hyTvTCK2DBfTwY+CLULcwtTU="></latexit>

   En el ejemplo (ver resolución en el libro):
   D2 = 9.51, K = 10. Si usamos un nivel de
   significancia del 1%, entonces t = 21.7.
   Como D2 < t, entonces aceptamos la hipótesis
   diciendo que los datos responden a una
   distribución uniforme con un error del 1%.




** Caso continuo

   16

   Opción 1: dividir el espacio en K subintervalos mutuamente exclusivos y calcular la
   probabilidad asociada a cada sub-intervalo
   (ver Ejemplo 8.36, libro Leon-García [15] donde
   se aplica un test Chi2 con una distribución
   exponencial usando 20 sub-intervalos).

   Opción 2: Test Estadístico Kolmogorov-Smirnov

   Dada n observaciones {x_1, x_2, \dots, x_n} (ordenadas) de una variable continua X, obtenemos una
   aproximación empírica a la función acumulativa:

   <latexit sha1_base64="Ycf//4GX99SVzRFpx5jZ3aFVh1U="></latexit>

   Método: medimos la distancia a la distribución real:
   <latexit sha1_base64="vx21Sm94RHFF2TrK9UqcprJU+5M="></latexit>

   Debemos rechazar la hipótesis H0 si q > c, el cual se
   determina a partir de un nivel de significancia:

   x1

   <latexit sha1_base64="3WONoTNXvdTvUGytop1DfX0hJdA="></latexit>

   Conclusión: se acepta H0 si
   (ver libro Papoulis [14], Cap. 9.3)

   x2

   x3 x4

   x5

   <latexit sha1_base64="MhiFY53QKsK8dW+GvpfXX6ZSMsA="></latexit>





** Tests empíricos: Test Espectral

   17

   Consiste en graficar secuencias de 2, 3 o más números consecutivos generados y
   detectar si existe algún patrón. Números realmente aleatorios no deberían exhibir
   ningún patrón.

   By Skylord, CC BY-SA 4.0, https://commons.wikimedia.org/w/index.php?curid=53864518

   En 3 dimensiones: Ejemplo IBM’s RANDU

   Xn+1 = (65539Xn )

   mod 231

   <latexit sha1_base64="fAbfraIrdLwdTXJntyCjUdE6ei8="></latexit>

   Luis Sanchez
   https://commons.wikimedia.org/w/index.php?curid=3832343





** Test Visual (Espectral)

   18

   ZOOM eje x





** Tests empíricos: Otros tests
   (se verán en la práctica)

   19

   - Test Serial: En una secuencia de números al azar busca comprobar que
   los pares de números consecutivos estén distribuidos uniformemente.
   - Run Test: Este test permite detectar secuencias monótonas de números
   al azar.
   - Gap Test: Siendo Ui un número aleatorio con distribución uniforme en un
   rango [0,1] y 0<=α < β <= 1. Este test consiste en contar la cantidad de
   números aleatorios generados de forma consecutiva entre la generación de
   un número que pertenece al intervalo [α,β] y el próximo número generado
   que vuelve a pertenecer al intervalo.

* Material de Lectura
   Libros (mínimo):
   [15] Probability, Statistics, and Random Processes for Eletrical Engineerging, 3rd_Ed. - Leon-Garcia. Caps. 2-3, 8.7.
   [4] Numerical Computing with Matlab - Cleve Moller. Cap. 9 (Pags. 282 - 295).

   Libros (opcional):
   [3] The Art of Computing Programming 3rd Ed 1997 - D. E. Knuth. Caps. 3.1, 3.2 y 3.3.
   [14] Papoulis-probability-random-variables-and-stochastic-processes. Caps. 2-4.

   Papers (opcional):
   - Lewis-Goodman-Miller, A pseudo-random number generator for the System
     360, 1969. Park and Miller, Random Number Generator Good Ones are Hard to
     Find, 1988. Mersenne Twister, A 623-dimensionally equidistributed uniform
     pseudorandom number generator, 1998.
 
 
 
 
 

