...
tambien transfieren el control del proceso de usuario al kernel. Una
interrupcion (por timer o no) no implica que el programa en ejecucion tiene un
error; en la mayoria de los casos, luego de resetear el timer, el so continua la
ejecucion del proceso, configurando su contador de programa y registros a los
