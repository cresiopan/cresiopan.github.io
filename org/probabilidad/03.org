...
Sea X una variable}
aleatoria absolutamente continua con densidad de probabilidades f
X
(x). La esperanza de X, 
...
**** Teorema 1.11. 
Sea X una variable aleatoria no negativa (i.e., F
X
(x) = \mathbb{P}(X \leq x) = 0 para
todo x < 0). Vale que
E[X] =}
Z
\infty
...
se describe mediante \{T
1
> 1{\}. Queremos hallar el máximo \lambda > 0 tal que \mathbb{P}(T
1
> 1) \geq 2}/{3.
...
El tiempo de funcionamiento de la máquina por jornada laboral es T = mín\{T
1
, 1{\}. Para}
t > 0 vale que}
F
T
(t) = \mathbb{P}(T \leq t) = 1 −\mathbb{P}(T > t) = 1 − \mathbb{P}(mín\{T
1
, 1{\} > t ) 
= 1 − \mathbb{P}(T}
1
...
Derivando F}
Y
(y) obtenemos la densidad de probabilidad de Y : f
Y
(y) =
1
3
...
x{\inA}
g ( x)\mathbb{P}(X = x ) . (8)
(b) Si X es continua con densidad de probabilidad f
X
(x) y g(X) es continua, entonces
E[g(X)] =}
Z
\infty
−\infty
...
g(x)f
X
(x) dx en el caso continuo, 
donde, según sea el caso, p
X
(x) y f
X
(x) son la función de probabilidad y la densidad conjunta
...
Sean X e Y dos variables aleatorias de varianzas finitas}
definidas sobre el mismo espacio de probabilidad (\Omega, \mathcal{A},\mathbb{P}). La covarianza de X e Y se define
por
Cov ( X, Y ) := E[(X − E[X]) (Y − E[ Y ])]. (22)
** Cálculo
Proposición 3.2. Sean X e Y dos variables aleatorias definidas sobre el mismo espacio de}
probabilidad (\Omega, \mathcal{A},\mathbb{P}). Si los segundos momentos de las variables al eatorias X e Y son finitos,
se tiene que
...
Para variables aleatorias X, Y, Z y constantes a, valen las siguientes}
propiedades
1. Cov(X, X) = V(X),
2. Cov(X, Y ) = Cov(Y, X),
3. Cov(aX, Y ) = aCov(X, Y ),
...
marginales, l a identidad (23) puede ser útil para calcular la esperanza del producto:
E[XY ] = E[X]E[Y ] + Cov(X, Y ).
**** Nota Bene 
Si X e Y son independientes, Cov(X, Y) = 0 porque E[XY ] = E[X]E[Y ]. Pero
...
i
A
1\{X \in A\} \leq \varphi ( X ) 1\{X \in A\} \leq \varphi ( X ) 
...
aleatorias independientes idénticamente distribuidas, tales que V(X_1
) < \infty} . Sea S}
n
, n \geq 1, la
sucesión de las sumas parciales definida por S}
n
:=
P
n
...
X
i
= 1 significa que /"en el i-ésimo ensayo ocurrió el evento A"/y la suma parcial S}
n
=
P
n
