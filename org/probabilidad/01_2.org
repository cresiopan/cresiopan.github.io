...
1. Definimos $\mathcal{A}_0$ como la familia de los intervalos
   ...
   definimos $P_0 : \mathcal{A}_0 \rightarrow \Re$ de la siguiente
   manera: $$P_0 (A) := longitud(A) = b − a$$, si los extremos del
...
2. La familia $\mathcal{A}_1$ de todas las uniones finitas de
   ...
   $$P_1(A) := \displaystyle\sum_{i=1}^k P_0(A_i), \text{ si }A =
...
3. El teorema de extensión se ocupa del resto: la medida de
   probabilidad $P_1$ definida sobre el álgebra $\mathcal{A}_1$ se
...
$$P_1(\ell(\omega) \geq \sqrt{3}) = \frac{\pi − \pi/4}{\pi} = \frac{3}{4}$$
...
$$P_2(A) = \frac{longitud(A)}{longitud(\Omega)} = \frac{longitud(A)}{2
\pi}$$

C y D se construyen del siguiente modo: Se fija el punto C; con la ley
$P_2$ se sortea un punto $\omega$ sobre el círculo de radio 1 y se
...
$$P_2(\ell(\omega) \geq \sqrt{3}) = \frac{2\pi / 3}{2 \pi} = \frac{1}{3}$$
...
es una densidad de probabilidades sobre $\Re^2$
denominada gaussiana bidimensional. En efecto,

#+name:eq:13
\begin{align*}
\iint_{\Re^2} 2 \pi \rho(x, y) dx dy &= \iint{\Re^2} exp\left(−\frac{x^2 + y^2}{2}\right)dxdy\\
